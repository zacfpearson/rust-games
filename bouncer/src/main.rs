...
fn main() {
    App::build()
        ...
        .insert_resource(BallController { since_last_ball: 0 })
        .insert_resource(ClearColor(Color::rgb(0.9, 0.9, 0.9)))
        .add_startup_system(setup.system())
        .add_system(ball_generating_system.system())
...
fn ball_generating_system(
    mut commands: Commands,
    mut ball_controller: ResMut<BallController>,
    mut scoreboard: ResMut<Scoreboard>,
    mut materials: ResMut<Assets<ColorMaterial>>,
) {
    let mut rng = rand::thread_rng();
    let x_vel_comp = rng.gen_range(-1.0..1.0);

    if ball_controller.since_last_ball > 20 {
        scoreboard.score += 1;
        commands
            .spawn_bundle(SpriteBundle {
                material: materials.add(Color::rgb(1.0, 0.5, 0.5).into()),
            ...
            .insert(Ball {
                velocity: 400.0 * Vec3::new(x_vel_comp, -0.5, 0.0).normalize(),
            });
        ball_controller.since_last_ball = 0;
    } else {
        ball_controller.since_last_ball += 1;
    }
...
fn ball_collision_system(
    ...
    for (ball_entity, mut ball, ball_transform, sprite) in ball_query.iter_mut() {
